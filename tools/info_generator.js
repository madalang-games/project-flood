...
function main() {
  ...
  const domainStaticDataDir = path.join(cfg.root, 'server', 'src', 'ProjectLink.Domain', 'StaticData');
  const ifaceFilePath       = path.join(cfg.root, 'server', 'src', 'ProjectLink.Domain', 'Interfaces', 'IStaticDataService.g.cs');
  const serviceFilePath     = path.join(cfg.root, 'server', 'src', 'ProjectLink.Infrastructure', 'Data', 'StaticDataService.g.cs');
