...
| `StageAttemptService.ClearAsync` | method | Clears attempt, refunds life, grants stage clear reward, sets Redis double_reward_eligible TTL 5min |
...
- Stage clear reward: look up `stage.reward_group_id` → `RewardService.GrantRewardGroupAsync` → set Redis eligibility key.
...
- Depends on: `shared/datas/stage/stage.csv` (reward_group_id)
